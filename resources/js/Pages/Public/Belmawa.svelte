<script lang="ts">
    ...
    const belmawaPrograms = [
        {
            title: 'PKM (Program Kreativitas Mahasiswa)',
            desc: 'Wadah bagi mahasiswa untuk mempraktikkan ilmu dan teknologi dalam bentuk riset, pengabdian, karya cipta, dan kewirausahaan.',
            tag: 'Kompetisi Ilmiah',
            color: 'from-amber-500 to-orange-600',
        },
        {
            title: 'P2MW (Program Pembinaan Mahasiswa Wirausaha)',
            desc: 'Program pendampingan dan pendanaan usaha bagi kelompok mahasiswa yang memiliki ide atau bisnis berjalan.',
            tag: 'Kewirausahaan',
            color: 'from-emerald-500 to-teal-600',
        },
        {
            title: 'Pilmapres (Pemilihan Mahasiswa Berprestasi)',
            desc: 'Ajang kompetisi pemilihan mahasiswa terbaik yang unggul secara akademik, capaian prestasi, dan kepemimpinan.',
            tag: 'Apresiasi Talenta',
            color: 'from-purple-500 to-indigo-600',
        },
        {
            title: 'PPK Ormawa (Program Penguatan Kapasitas Ormawa)',
            desc: 'Program pengabdian masyarakat berbasis organisasi mahasiswa untuk pemberdayaan desa binaan.',
            tag: 'Pengabdian Desa',
...
<svelte:head>
    <title>Program Belmawa - Kemahasiswaan & Alumni UNU Purwokerto</title>
</svelte:head>
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-emerald-500 selection:text-white transition-colors duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                ...
                <nav class="hidden lg:flex items-center gap-1 text-xs font-semibold">
                    ...
                    <Link href="/belmawa" class="px-3.5 py-2 rounded-xl font-extrabold text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-500/30 shadow-sm">Belmawa</Link>
    ...
    <!-- Page Banner -->
    <div class="relative py-16 bg-gradient-to-r from-amber-500/10 via-orange-500/5 to-slate-50 dark:from-amber-950/60 dark:via-slate-900 dark:to-slate-950 border-b border-slate-200/80 dark:border-slate-800 bg-grid-pattern transition-colors">
        <div class="absolute -top-32 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-amber-500/15 dark:bg-amber-500/20 blur-[120px] rounded-full pointer-events-none animate-pulse-glow"></div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <div class="max-w-3xl space-y-4">
                <div class="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-amber-500/10 dark:bg-amber-500/20 border border-amber-500/30 text-amber-700 dark:text-amber-300 text-xs font-black tracking-wider uppercase animate-float">
                    <Sparkles class="w-4 h-4 text-amber-500" />
                    Program Belmawa Kemendiktisaintek
                </div>
                <h1 class="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight text-slate-900 dark:text-white leading-tight">
                    Kompetisi & Hibah Pembinaan Mahasiswa
                </h1>
                <p class="text-slate-600 dark:text-slate-300 text-base sm:text-lg leading-relaxed">
                    Fasilitas pendampingan, coaching clinic, dan apresiasi pendanaan proposal PKM, P2MW, Pilmapres, dan PPK Ormawa bagi mahasiswa UNU Purwokerto.
                </p>
            </div>
        </div>
    </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            {#each belmawaPrograms as program}
                <div class="bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-8 shadow-md hover:shadow-xl hover:border-amber-500/40 transition-all flex flex-col justify-between group hover:-translate-y-1">
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            <span class={`px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider text-white bg-gradient-to-r ${program.color} shadow-sm`}>
                                {program.tag}
                            </span>
                        </div>
                        <h3 class="text-xl font-extrabold text-slate-900 dark:text-slate-100 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
                            {program.title}
                        </h3>
                    ...
                    <div class="pt-6 mt-6 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                        <span class="text-xs text-emerald-600 dark:text-emerald-400 font-extrabold flex items-center gap-1">
                            <CheckCircle2 class="w-4 h-4" /> Pendampingan Dosen Pembimbing
                        </span>
                        <Link href="/informasi?kategori=program-belmawa" class="inline-flex items-center gap-1 text-xs font-extrabold text-amber-600 dark:text-amber-400 hover:underline">
                            Detail Program <ArrowRight class="w-3.5 h-3.5" />
                        </Link>
                    </div>
                </div>
            {/each}
