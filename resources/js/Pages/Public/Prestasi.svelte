<script lang="ts">
    ...
    let { achievements = { data: [] } }: Props = $props();
    ...
    const staticAchievements = [
        {
            title: 'Juara 1 Program Belmawa PKM Riset Eksakta Nasional 2026',
            student: 'Tim Mahasiswa Informatika & FMIPA',
            event: 'Kemendiktisaintek RI',
            medal: 'Medali Emas',
            level: 'Tingkat Nasional',
            color: 'from-amber-500 to-yellow-600',
        },
        {
            title: 'Juara 2 Kompetisi Wirausaha Mahasiswa P2MW 2026',
            student: 'Tim Bisnis Agroteknologi UNU Purwokerto',
            event: 'Direktorat Belmawa 2026',
            medal: 'Medali Perak',
            level: 'Tingkat Nasional',
            color: 'from-slate-400 to-slate-500',
        },
        {
            title: 'Juara 1 MTQ Mahasiswa Cabang Karya Tulis Ilmiah Al-Qur\'an',
            student: 'Ahmad Fauzi & Tim PAI UNU Purwokerto',
            event: 'MTQ Mahasiswa Nasional VI',
            medal: 'Medali Emas',
            level: 'Tingkat Nasional',
            color: 'from-amber-500 to-yellow-600',
        },
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-emerald-500 selection:text-white transition-colors duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                ...
                <nav class="hidden lg:flex items-center gap-1 text-xs font-semibold">
                    ...
                    <Link href="/prestasi" class="px-3.5 py-2 rounded-xl font-extrabold text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-500/30 shadow-sm">Prestasi</Link>
    ...
    <!-- Page Banner -->
    <div class="relative py-16 bg-gradient-to-r from-purple-500/10 via-indigo-500/5 to-slate-50 dark:from-purple-950/60 dark:via-slate-900 dark:to-slate-950 border-b border-slate-200/80 dark:border-slate-800 bg-grid-pattern transition-colors">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <div class="max-w-3xl space-y-4">
                ...
                <p class="text-slate-600 dark:text-slate-300 text-base sm:text-lg leading-relaxed">
                    Etalase kebanggaan bagi mahasiswa peraih kejuaraan riset, teknologi, olahraga, seni budaya, dan keagamaan di tingkat Wilayah, Nasional, & Internasional.
                </p>
            </div>
        </div>
    </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            {#each staticAchievements as ach}
                <div class="bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-6 shadow-md hover:shadow-xl hover:border-purple-500/40 transition-all flex flex-col justify-between group hover:-translate-y-1">
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            <span class={`px-3 py-1 rounded-full text-[10px] font-extrabold uppercase text-white bg-gradient-to-r ${ach.color} shadow-sm`}>
                                {ach.medal}
                            </span>
                            <span class="text-[10px] font-bold text-purple-600 dark:text-purple-400 bg-purple-500/10 px-2.5 py-0.5 rounded-full border border-purple-500/20">
                                {ach.level}
                            </span>
                        </div>
                        <h3 class="text-base font-extrabold text-slate-900 dark:text-slate-100 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors leading-snug">
                            {ach.title}
                        </h3>
